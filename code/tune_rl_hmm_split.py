# Script 1: Explore RL/HMM Weight Combinations
# This script tests different weightage splits between RL and HMM to find the optimal balance
...
SPLITS_TO_TEST = [
    # (0.0, 1.0),   # Pure HMM
    (0.05, 0.95), # 5% RL, 95% HMM
    (0.1, 0.9),   # 10% RL, 90% HMM
    (0.15, 0.85), # 15% RL, 85% HMM
    (0.2, 0.8),   # 20% RL, 80% HMM
    # (0.3, 0.7),   # 30% RL, 70% HMM
    # (0.4, 0.6),   # 40% RL, 60% HMM
    # (0.5, 0.5),   # Equal split
    # (0.6, 0.4),   # 60% RL, 40% HMM
    # (0.7, 0.3),   # 70% RL, 30% HMM
    # (0.8, 0.2),   # 80% RL, 20% HMM
    # (0.9, 0.1),   # 90% RL, 10% HMM
    # (1.0, 0.0),   # Pure RL
]

TRAINING_EPISODES = 2000  # Faster training for grid search
EVAL_SAMPLE_SIZE = 500    # Smaller eval set for speed
...
def select_action(policy_net: DQN, state_vec: np.ndarray, guessed_letters: set, eps: float, device: torch.device, 
                  hprob: Optional[np.ndarray] = None, masked_word: Optional[str] = None, rl_weight: float = 0.1, hmm_weight: float = 0.9):
    """Modified to accept custom RL/HMM weights"""
...
def train_dqn(corpus_words: List[str], oracle: LengthSpecificNGramOracle, test_words: List[str],
              ...
              device: Optional[torch.device] = None, rl_weight: float = 0.1, hmm_weight: float = 0.9):
    if device is None:
    ...
    for ep in range(1, num_episodes+1):
        ...
        while not done:
            ...
            act = select_action(policy_net, state_vec, guessed, eps, device, hprob=hprob, masked_word=masked, 
                              rl_weight=rl_weight, hmm_weight=hmm_weight)
...
def evaluate_agent(policy_net: DQN, oracle: LengthSpecificNGramOracle, test_words: List[str], max_wrong: int = 6, 
                   device: Optional[torch.device]=None, rl_weight: float = 0.1, hmm_weight: float = 0.9, verbose: bool = False):
    if device is None:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
...
if __name__ == '__main__':
    ...
    print("="*80)
    print("RL/HMM WEIGHT EXPLORATION")
    print("="*80)
    ...
    print(f"Oracle trained for lengths: {sorted(oracle.length_models.keys())}")
    ...
    for rl_w, hmm_w in SPLITS_TO_TEST:
        print(f"\n{'='*80}")
        print(f"Testing: RL={rl_w:.2f}, HMM={hmm_w:.2f}")
        print(f"{'='*80}")
        
        # Train
        print(f"Training for {TRAINING_EPISODES} episodes...")
        policy_net, rewards = train_dqn(corpus_words, oracle, test_words, 
                                       num_episodes=TRAINING_EPISODES, batch_size=128, 
                                       max_wrong=6, device=device, 
                                       rl_weight=rl_w, hmm_weight=hmm_w)
        ...
        eval_results = evaluate_agent(policy_net, oracle, eval_sample, max_wrong=6, 
                                     device=device, rl_weight=rl_w, hmm_weight=hmm_w)
    ...
    print(f"{'='*80}\n")
    print(f"{'RL':>6} {'HMM':>6} {'Success':>10} {'Wrong':>8} {'Repeats':>8} {'Score':>10} {'Train Reward':>12}")
    print(f"{'-'*80}")
    
    for r in results:
        print(f"{r['rl_weight']:>6.1f} {r['hmm_weight']:>6.1f} "
              f"{r['success_rate']:>9.1%} {r['total_wrong']:>8d} "
              f"{r['total_repeats']:>8d} {r['final_score']:>10.2f} "
              f"{r['avg_training_reward']:>12.2f}")
    ...
    print(f"{'='*80}")
    print(f"RL Weight: {best['rl_weight']:.1f}")
    print(f"HMM Weight: {best['hmm_weight']:.1f}")
    print(f"Success Rate: {best['success_rate']:.1%}")
    print(f"Final Score: {best['final_score']:.2f}")
    
    # Save results
    output_file = 'rl_hmm_split_results.json'
